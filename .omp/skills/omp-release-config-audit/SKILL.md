...
- When no user boundary is supplied, use `.omp/skills/omp-release-config-audit/reviewed-through` as the inclusive reviewed-through baseline only when it is a regular tracked file containing exactly one nonempty valid SemVer line with a trailing newline. Validate its version against immutable package/release evidence and require that the baseline not exceed the target; then research releases newer than that baseline through the target. The sidecar covers release research for the whole managed OMP surface; it never replaces a fresh current-schema/effective-state audit. A missing, malformed, or unavailable sidecar is suspect and requires an explicit boundary.
