...
2. If the user did not supply a boundary, inspect `.omp/skills/omp-release-config-audit/reviewed-through`. Use it as the inclusive reviewed-through baseline only when it is a regular tracked file containing exactly one nonempty valid SemVer line with a trailing newline. Validate the version against immutable package/release evidence and require that the baseline not exceed the target; then use versions greater than the baseline and less than or equal to the target as the release-research window. The sidecar is an inclusive reviewed-through baseline for the entire managed OMP surface—not an installed-version claim—and never skips a fresh schema, migration, effective-state, extension, documentation, or working-tree audit. Treat a missing, malformed, unavailable, or target-newer sidecar as suspect and require an explicit boundary rather than trusting it.
3. If neither a valid sidecar nor a user range exists, require the audit scope to state an explicit boundary; do not silently infer one from local evidence.
...
7. Never advance the sidecar for a partial, blocked, or report-only audit. Update it only with explicit authorization after the target range is complete and material findings are resolved or consciously accepted.
