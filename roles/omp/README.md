...
- Both managed `config.yml` files contain intentional non-default overrides and explicit reproducibility or behavior pins. Omit a setting when the desired value equals the current default unless a deliberate reproducibility or behavior pin is intended. Re-audit after OMP upgrades because inherited behavior follows upstream default changes.
- Repo-managed `modelRoles` keep `openai-codex/gpt-5.6-sol:xhigh` for normal user-facing `default` and `plan` work. Normal delegated work uses unpinned `openai-codex/gpt-5.6-terra` for `task`, allowing the bundled task agent's automatic effort selection and the repo-managed specialists' declared `thinkingLevel`; `slow` uses Terra `xhigh`, while `designer` and `vision` use Terra `high`. Unpinned `openai-codex/gpt-5.6-luna` for `smol` preserves the bundled scout, sonic, and librarian effort declarations; `tiny` is explicitly Luna `low`, `commit` is Luna `medium`, and the high-volume normal `advisor` is `cursor/cursor-grok-4.6-high`. The deep-review config keeps Terra `xhigh` for `default`, `slow`, `plan`, `designer`, and `vision`, leaves `task` unpinned for adaptive effort, shares the normal profile's `smol`, `tiny`, and `commit` policy, and uses Terra `low` for `advisor`. The normal config enables its main-session advisor but configures no subagent advisors; its explicit `tier.openai` is `default`, and its `tier.subagent` and `tier.advisor` are both `priority`. Deep-review and local-max each set `task.agentAdvisor` to `on` for every currently bundled agent (`scout`, `designer`, `reviewer`, `security-reviewer`, `librarian`, `task`, and `sonic`) and repo-managed agent (`gap-advisor`, `plan-critic`, `risk-assessor`, `validator`, and `security-auditor`). This is an explicit-name policy: add every future bundled or repo-managed agent to both maps.
...
- `agents/*.md` defines additional global OMP agents with OMP frontmatter only: `name`, `description`, optional `tools`, optional `model`, optional `thinkingLevel`, optional `read-summarize`, then the system prompt body. Each repo-managed specialist routes through `model: "@task"` so the configured task model is used with that specialist's own thinking level instead of inheriting the parent session model.
...
`omp-local-max` is fully local during normal role resolution: every supported model role resolves to exact selector `ollama/qwen3.6:35b-omp`, `enabledModels` permits only that selector, and the quick-cycle contains only local roles. Substantive, default, task, and advisor work uses maximum thinking; only `smol`, `tiny`, and `commit` deliberately use lower effort. Its main-session advisor synchronizes after one-turn backlog and has no interrupt immunity; `task.agentAdvisor` explicitly enables advisor coverage for the current bundled and repo-managed agent names listed above. These settings prevent automatic resolution or fallback from spending hosted-model tokens; an explicit CLI override such as `--model openai-codex/...` remains an intentional escape hatch.
