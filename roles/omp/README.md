...
- `extensions/*` is deployed as per-file symlinks into `~/.omp/agent/extensions/`. Never symlink the whole extensions directory. Preserve unrelated user-installed extension files. Cleanup may remove only stale symlinks whose target points into this repo's managed extension source and whose source file no longer exists.
