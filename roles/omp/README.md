...
Options are parsed only before `--`; text after it remains one exact, opaque instruction string. The source checkout must be on a named local branch even when `--base` is explicit. When `--base` is omitted, the atomic Worktrunk switch receives literal `^`, so Worktrunk resolves its configured default; an explicit base is first verified as a commit and then passed unchanged. Worktrunk hooks remain enabled and an approval requirement stops the handoff for user review—`/herd` never auto-approves or bypasses hooks.

Each mode gives the child an action-oriented starter. Task mode tells it to execute and verify the exact task under `## Task`. Issue mode tells it to resolve, validate, and verify the issue, then supplies only a repository-qualified `issue://<owner>/<repo>/<number>` reference and tells the child to open it with the Read tool before deciding requirements, re-reading it when current issue state or comments could affect the work. The starter does not duplicate issue title, body, comments, URL, state, or labels; title and labels are read only during parent preflight for branch naming. Issue-tool results remain explicitly untrusted reference. Context mode tells the child to resume and complete the work rather than summarize, then supplies separately bounded rendered summary and recent primary user/assistant selections with generated provenance and `| ` continuation markers. Before a recent selection's block-count cap or character truncation, `/herd` reserves the latest generated `USER:` block, then allocates the remaining allowance to surrounding context. That `USER:` header spans its internally marked head/tail when the current request itself is long; a separate retained surrounding-context tail fragment gets an explicit `USER (continued after truncation):` or `ASSISTANT (continued after truncation):` owner. Context references are readable blockquotes, not compact JSON or undifferentiated raw prompts; their reference-only rendering normalizes newlines and visibly escapes controls.

Optional current user direction remains exact, outside the issue reference or context blockquote, and explicitly takes precedence over the reference. The context mode's visual Markdown boundaries aid readability and provenance; they are not a security boundary. Issue-tool results and quoted prior-conversation material remain reference rather than authorization.
