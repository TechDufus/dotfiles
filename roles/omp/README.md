...
Each mode gives the child an action-oriented starter. Task mode tells it to execute and verify the exact task under `## Task`. Issue mode tells it to resolve, validate, and verify the issue, then supplies readable blockquoted `Repo`, `Number`, `Title`, `URL`, `State`, `Labels`, and `Body` fields with an explicit untrusted-reference warning. Context mode tells it to resume and complete the work rather than summarize, then supplies separately bounded rendered summary and recent primary user/assistant selections with generated provenance and `| ` continuation markers. Before a recent selection's block-count cap or character truncation, `/herd` reserves the latest generated `USER:` block, then allocates the remaining allowance to surrounding context. That `USER:` header spans its internally marked head/tail when the current request itself is long; a separate retained surrounding-context tail fragment gets an explicit `USER (continued after truncation):` or `ASSISTANT (continued after truncation):` owner. Issue and context references are readable blockquotes, not compact JSON or undifferentiated raw prompts; their reference-only rendering normalizes newlines and visibly escapes controls.

Optional current user direction remains exact, outside the blockquote, and explicitly takes precedence over the reference. The visual Markdown boundaries aid readability and provenance; they are not a security boundary, so quoted issue and prior-conversation material remains reference rather than authorization.
