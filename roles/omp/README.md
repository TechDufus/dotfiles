...
Run `/herd done` from the OMP agent that `/herd` started after its exact local `HEAD` has been merged through a GitHub pull request. The command accepts no options. It is intentionally unavailable to pre-existing OMP processes and agents created by older extension versions because `/herd` injects one cleanup-ownership ledger into the new agent's environment at startup.
...
OMP's advisor serialization omits hidden custom messages, so the hidden `commit-request` alone did not show that the user had invoked the registered command and could make its immediate `omp_commit` call appear unauthorized. The command emits a visible, user-attributed `[commit-authorization]` runtime record after activating `omp_commit` alongside the existing tools and immediately before the hidden request. The advisor sees it as a standalone, role-unlabeled history entry, distinct from ordinary user- or model-authored text. That marker authorizes exactly the following one `omp_commit` workflow; the extension also consumes a matching in-memory one-use authorization before execution, so absent or repeated calls fail before Git work. The marker neither triggers a turn nor authorizes direct, unrelated, or later tool calls. The detailed `commit-request` remains hidden and is still the only message that triggers the next turn. Tool completion restores the exact previous active-tool list; failed restoration remains retryable without removing the normal tools.
