...
equal(noModelOverride, { mode: "context", dryRun: false, instructions: "" }, "blank must alias context");
...
equal(
  ...
  { mode: "context", dryRun: false, model: fullOpenAiSelector, effort: "xhigh", instructions: "" },
  "full OpenAI model selector parse changed",
);
equal(
  parseHerdArgs("task --model=vendor:preview/gpt-5.6-terra:high -- exact task"),
  { mode: "task", dryRun: false, model: "vendor:preview/gpt-5.6-terra", effort: "high", instructions: "exact task" },
...
equal(
  ...
  { mode: "issue", issue: "owner/repo#123", dryRun: false, model: fullOpenAiSelector, effort: "auto", instructions: "" },
...
for (const effort of ["off", "minimal", "low", "medium", "high", "xhigh", "max", "auto"]) {
  equal(
    ...
    { mode: "context", dryRun: false, model: fullOpenAiSelector, effort, instructions: "" },
...
async function success() {
  ...
  equal(start.options.timeout, 35_000, "agent start wrapper deadline must exceed the ordinary CLI timeout");
...

{
  ...
  for (const alias of ["--help", "-h", "help"]) {
    ...
    for (const required of [
      ...
      "--branch=<name>", "--base=<ref>", "--model=<model-selector>:<effort>", "--dry-run",
