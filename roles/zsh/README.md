...
On Linux and macOS, queued recipe reads resolve at each await as serial `op run` groups per 1Password account, rather than individual background `op read` processes. The current recipe makes one direct AWS item-name read followed by two such waves—three 1Password operations that reduce desktop approval prompts, without guaranteeing a single approval.
